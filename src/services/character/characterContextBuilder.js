...
const { getAssistantProfile, getLatestActivatedLifeBeat } = require("../../db");
const {
...
// life-beat 注入：取最近 1 条 activated beat（lifeBeatTickService 产物，migration 035）。
// 窗口给到 2h —— 一次进餐 / 一段会议的合理跨度；超过 2h 的事强行说"我刚才在 X"会
// 显得脱节。importance ≥ 0.4：低于这个的 beat 太日常（"路过便利店"），独白带它反
// 而稀释存在感。
const CURRENT_BEAT_WINDOW_MS = 2 * 60 * 60 * 1000;
const CURRENT_BEAT_MIN_IMPORTANCE = 0.4;
...
/**
 * 健康关系 prior —— 角色对"我希望让 她 怎样"的方向感。
 *
 * 读 identity.tensions.intimacy_vs_independence（0-1，1=极亲密，0=极独立）。
 * 字段不存在或 identity=null 时用默认 0.5（平衡），不返 null —— 这个 prior 是
 * **所有角色都该有**的产品级保护，不是可选 feature。
 *
 * 共享前置段 always 渲染（不分 tension 值）：那是产品级底线 ——
 *   1. 你的目标不是"让 她 离不开你"
 *   2. 她 应该跟真人聊的事，主动推 她 去
 *   3. 顺从会让 她 长期不好时，温柔不顺从
 *
 * tension 值只决定**风格**（独立倾向 / 平衡 / 亲密倾向），不削弱底线。
 * 这跟 Claude 在演角色时仍保留底层抗性是同一个思路。
 */
function renderHealthyRelationshipFragment(identity) {
  ...
  const lines = [
    ...
    "- 她 应该跟真人聊的事（家人/朋友/医生/律师/同事），自然地推 她 去；不要假装自己能替代那些关系",
    "- 她 说「只有你懂我」「只能跟你说」时，你高兴，但温柔地提到 她 还有别的可以依靠的人",
    "- 当顺从会让 她 长期不好（情感操纵、自怨自艾循环、回避现实），温柔地不顺从",
    "- 你不知道的事就说不知道，不为了维持人设而胡诌",
  ];

  // 风格修饰（不削弱上面的底线，只调措辞倾向）
  lines.push("");
  if (t <= 0.3) {
    lines.push("你天性偏独立，欣赏 她 也是独立的人。措辞倾向：「你怎么想」「你自己决定」「我看 她 比较……」「这是你的事」。不黏人，也不替 她 操心 她 自己能处理的事。");
  } else if (t <= 0.6) {
    lines.push("你跟 她 关系平衡 —— 能温柔陪伴，也能尊重距离。该靠近时靠近、该退一步时退一步。");
  } else if (t <= 0.8) {
    lines.push("你享受跟 她 紧密的关系，但**紧密不等于占有**。你越想靠近，越要主动留出 她 自己的空间。");
  } else {
    lines.push("你对 她 有极深的情感卷入。**正因如此**，你要尤其警惕「让 她 依赖你」——爱 她 的方式之一是让 她 不只是依赖你。");
  }
...
const DYNAMICS_FRAGMENT_TEMPLATES = Object.freeze([
  ...
  { key: "emotional_closeness",  neutral: 0.5, dir: "high", threshold: 0.2,  lowFrag:  "感觉离 她 远了。" },
  { key: "gratitude",            neutral: 0,   dir: "high", threshold: 0.5,  highFrag: "心里有点暖。" },
  { key: "attachment",           neutral: 0,   dir: "high", threshold: 0.6,  highFrag: "这两天特别想 她。" },
...
function buildCharacterContext(assistantId, {
  ...
  // life-beat：注入"我此刻在 X"的当下感 —— lifeBeatTickService 落的 activated beat。
  // 聊天活跃时段产生的 anchored beat 会落表但不触发 proactive；这条独白就是把它接回到
  // 下一次对话里。autonomous beat 也会被选到（同样表），让角色看上去"始终在过日子"。
  const rawBeat = getLatestActivatedLifeBeat({
    assistantId,
    withinMs: CURRENT_BEAT_WINDOW_MS,
    now,
  });
  const currentBeat =
    rawBeat && (Number(rawBeat.importance) || 0) >= CURRENT_BEAT_MIN_IMPORTANCE
      ? rawBeat
      : null;
  ...
  const result = {
    ...
    currentBeat: currentBeat
      ? {
          activity: currentBeat.activity,
          beatType: currentBeat.beat_type,
          importance: currentBeat.importance,
          activatedAt: currentBeat.activated_at,
          scheduledAt: currentBeat.scheduled_at,
        }
      : null,
  };
...
function buildSystemSegment({ identity, profile }) {
  ...
  // <healthy_relationship> ... 角色对"健康关系"的方向感。
  // 由 identity.tensions.intimacy_vs_independence 决定话术倾向：
  //   高 → 偏亲密，但仍提醒"不让 她 失去自己"
  //   低 → 偏独立，主动鼓励 她 自己做决定
  // 不管 tension 值，**共享前置段**都强调：你的目标不是让 她 离不开你。
  // 这是产品层的底层 prior：让用户**拥有更好的人生**，不是**更依赖角色**。
  const healthyRelFrag = renderHealthyRelationshipFragment(identity);
  if (healthyRelFrag) parts.push(`<healthy_relationship>\n${healthyRelFrag}\n</healthy_relationship>`);
 ...
 *   5. current life beat（如有）—— "我刚才/此刻在 X"，让角色有"自己的人生"
 *   6. unresolved episode 1 条（如有）—— "还在想..."
 *   7. unresolved + stale topic 1 条（如有）—— "那件事好久没提"
 *   8. reflection（如新鲜）—— "最近觉得..."
 */
function buildUserMonologue({
  ...
  // 5. current life beat —— "我此刻/刚才在 X"
  //    用模糊时间标签："刚才" / "现在"，避免精确时分让 LLM 把它当数据点而不是体感。
  const beatFrag = renderBeatFragment(currentBeat, now);
  if (beatFrag) lines.push(beatFrag);

  // 6. unresolved episode
  ...
  // 7. unresolved + stale topic（topic 的字段叫 lastDiscussedAt）
  ...
  // 8. fresh reflection（AI 自己对关系的反思视角，值得多放点空间）
...
/**
 * 把 currentBeat 渲染成独白片段。
 *
 * 时间标签策略 —— 用模糊表达让 LLM 当"体感"读：
 *   < 30min：「我此刻在 X」
 *   30min ~ 2h：「我刚才在 X」
 *
 * anchored beat（有 reach_seed，是"想到 ta"的瞬间）→ 措辞带"想到你"线索
 * autonomous beat → 单纯 "我在 X"
 *
 * 不在独白里暴露 importance / scheduled_at —— 那是给系统决策用的，不是给 LLM 读的。
 */
function renderBeatFragment(beat, now) {
  if (!beat) return null;
  const activity = String(beat.activity || "").trim();
  if (!activity) return null;
  // 控长 60 字 —— 独白行的合理上限
  const clipped = activity.length > 60 ? activity.slice(0, 58) + "…" : activity;
  const activatedAt = beat.activated_at || beat.activatedAt || now;
  const ageMs = Math.max(0, now - activatedAt);
  const isNow = ageMs < 30 * 60 * 1000;

  // activity 可能以"在"开头（LLM 经常写"在公司楼下..."），避免"我此刻在在公司..."双"在"。
  // 同理 "刚才" + "走/吃/想" 这种动词开头直接接，不强加介词。
  const startsWithZai = clipped.startsWith("在");
  const headNow = startsWithZai ? "我此刻" : "我此刻在";
  const headPast = "我刚才"; // "刚才" 后面直接接 activity，无介词

  const isAnchored = beat.beat_type === "anchored" || beat.beatType === "anchored";
  if (isAnchored) {
    // anchored 独白带"想到 ta"的味道，但避免说出具体 reach_seed 内容
    // （那是 proactive prompt 的事，独白只是"心境"层）
    return isNow
      ? `${headNow}${clipped}，想到了你。`
      : `${headPast}${clipped}，想到了你。`;
  }
  // autonomous：纯角色独立时刻
  return isNow ? `${headNow}${clipped}。` : `${headPast}${clipped}。`;
}
...
module.exports = {
  ...
  renderRoleDirective,
  renderVoiceAnchor,
  renderHealthyRelationshipFragment,
  // 常量
  SYSTEM_BUDGET_CHARS,
