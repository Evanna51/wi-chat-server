 ...
 *
 * 2026-05-24（migration 035）：删除 WATCHDOG_LIFE_EVENT_FAST_GAP_MS 路径。
 *   旧版"最近 1h 有 life_event 就把 gap 砍到 30min"是为补 catchupService 时代被动 life
 *   写入的反应慢；现在 lifeBeatTickService 在 anchored beat 落库时直接调 scheduleNextPushPlan(
 *   reason='life_event_seed') 主动触发，watchdog 不再需要为 life event 加速 —— 只做
 *   纯粹的 72h 续链兜底。
 *
...
const WATCHDOG_MIN_GAP_FROM_PROACTIVE_MS = 60 * 60 * 1000;  // 距上一条主动消息 1h
...
async function runProactiveWatchdogOnce({ now = Date.now() } = {}) {
  ...
  for (const { assistant_id: assistantId } of profiles) {
    ...
    if (sinceLastProactive < WATCHDOG_MIN_GAP_FROM_PROACTIVE_MS) {
      incSkip("min_gap_not_yet");
      continue;
    }
