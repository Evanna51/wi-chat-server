...
| `add_fact` | `memoryId`, `factKey`, `factValue`, `factConfidence?` (0-1, 默认 0.8), `factImportance?` (0-1, 默认 0.5) | 给该 memory 加 fact；`(memoryId, factKey)` 已存在时按 confidence 高低决定是否覆盖。`importance` 与 `confidence` 正交：前者 = "对角色行为影响多大"（健康/重大身份建议 0.9+，偏好兴趣 0.3-0.5），后者 = "提取得准不准"。bootstrap 的 `coreFacts` 按 `importance*0.6 + confidence*0.4` 综合排序 |
...
`message_create`（实时单条落库；替代批量 `/api/sync/push`）：
```json
{
  "op": "message_create",
  "id": "<turn-uuid-v7>",
...
```
Server 处理后回 `message_persisted`：
```json
{ "op": "message_persisted", "ok": true, "id": "<turn-id>", "status": "accepted|skipped|replaced|rejected", "reason": null, "ts": ... }
```
**触发**：`role=user` 落库后会 cancel 长期 plan + 异步 `scheduleNextPushPlan`（见 §16）。

`message_update`（编辑既有消息内容；同步 re-embed memory_item，不动 facts）：
```json
{ "op": "message_update", "id": "<turn-id>", "content": "<新内容>", "assistantId": "..." }
```
Server 回 `message_updated`：
```json
{ "op": "message_updated", "ok": true, "id": "<turn-id>", "memoryUpdated": 1, "ts": ... }
```
...
两种生成模式并存，用 `trigger_reason` 区分：

| 模式 | trigger_reason | 何时生成 | 何时使用 |
|------|---------------|----------|----------|
| **滚动单条 next_push（事件驱动）** | `next_push` | 每次 user/AI turn 落库后 + plan-executor 派发完后 | 用户上次回复 ≤ 72h（活跃窗口） |
| **长期 trigger（cron 驱动）** | `inactive_7d` / `daily_greeting` / `manual_request` | `PLAN_GENERATION_CRON` 周期触发 | 用户 > 72h 未回（沉默窗口） |

**不变量**：同一 (assistant, user) 同一时刻 **最多 1 条** pending `next_push`。任何新 turn 把它废掉、重算。

#### 16.1 next_push 触发链路

```
user 发消息 (HTTP /sync/push 或 WS message_create)
    ↓
ingestTurnsBatch 入库
    ↓
cancelPendingPlansForAssistant(aid, "user_active")  ← 清掉长期 plan
    ↓
scheduleNextPushPlan(aid)  ← async setImmediate，不阻塞 HTTP 响应
    ├─ 若 now - lastUserAt > 72h → skip 'past_72h_handover_to_long_term'
    ├─ 否则 cancel 旧 next_push pending → 调 LLM 决定 delayMs + body + intent
    │     LLM 上下文：character_background + coreFacts + lifeEvents
    │     + 最近 6 条 turns + 用户上一句 + 当前时间
    └─ LLM 也可主动 skip：{"skip":true,"skipReason":"用户在忙"} → 不插 plan
```

派发完一条 `next_push` 后（option A）：plan-executor 立刻再 `scheduleNextPushPlan`，AI 自己决定是连续推（"再分享件事"）还是 skip（"等用户主动"）。

#### 16.2 立即生成 plan（手动触发，走长期 trigger）

```
POST /api/proactive/regenerate-plans
body: { "assistantId": "...", "force": true }   // 不传则跑全部 allow_proactive_message=1 的角色
```

`force: true` 时绕过 trigger 评估，直接合成 `manual_request` plan，2 分钟后派发。
...
#### 16.5 cancelled_reason 速查

| reason | 含义 |
|--------|------|
| `replaced_by_new_turn` | 同 assistant 又来了新 turn，旧的 next_push 被 cancel |
| `user_active` | 用户主动发消息，长期 trigger plan 被 cancel（next_push 由 schedule 接手） |
| `past_72h_handover_to_long_term` | 72h 没用户回复，next_push 不再排，残留 pending 被清 |
| `scheduled_beyond_72h_window` | LLM 给的 delayMs 加上 lastUserAt 已超 72h 边界，丢弃 |
| `manual` | DELETE /api/proactive/plans/:id 手动取消 |
...
### 18) Character Bootstrap (session 启动 / 每日固定)

- `GET /api/character/bootstrap?assistantId=<id>`
- 客户端在 session 启动 / 每日固定调一次，一次拿齐拼 system prompt 的所有静态 + 会话级信息。
- 与 `POST /api/tool/memory-context` 区分：bootstrap **不做语义检索**，只取 profile / 状态快照 / pinned 关键记忆 / 高分 facts；memory-context 才是每轮对话的查询入口。
- `relationshipState` 在 `character_state` 行不存在时自动以默认值初始化（同 `/api/relationship/state` 行为），**永远不会返回 404**。
- `profile` 仅用于客户端对照本地缓存（客户端权威）：server 无 `assistant_profile` 行时为 `null`，客户端可据此触发 `/api/assistant-profile/upsert` 同步。
...
```json
{
  ...
  "profile": {
    ...
    "characterName": "Mira",
    "characterBackground": "...",
    "assistantType": "companion"
  },
  "relationshipState": { "...": "见 17) 同 schema" },
  "coreMemories": [
    {
      "id": "019dca12-...",
      "content": "和用户初见于 3/13 凌晨",
      "memoryType": "user_turn",
      "category": "relationship_info",
      "quality": "A",
      "salience": 0.95,
      "kbName": null,
      "createdAt": 1731000000000,
      "pinnedAt": 1731500000000
    }
  ],
  "coreFacts": [
    {
      ...
      "importance": 0.95,
      "score": 0.95,
      "memoryItemId": "019dfda1-...",
      "createdAt": 1731000000000
    },
    {
      "id": "019e005b-...",
      "factKey": "relationship_with_mom",
      "factValue": "医生",
      "confidence": 0.9,
      "importance": 0.8,
      "score": 0.84,
...
| 路径 | 含义 |
|------|------|
| `profile` | `null` 或 `{assistantId, characterName, characterBackground, assistantType}`。客户端用来对比本地缓存是否漂移 |
| `relationshipState` | 同 17) 节 schema；`null` 仅在 `ensureDefaultState` 兜底失败的极端情况 |
| `coreMemories[]` | `is_pinned=1` 的关键记忆（整段对话/事件原文），按 `(salience DESC, pinned_at DESC)` 排序，最多 8 条 |
| `coreFacts[]` | 高分 facts（key-value 形式的稳定事实），按综合分 `importance*0.6 + confidence*0.4 ≥ 0.55` 过滤 + DESC 排序，按 `factKey` 去重，最多 15 条 |
| `score` | 每条 fact 的综合分（已四舍五入到 3 位）。`importance` 主导（"对角色行为影响多大"），`confidence` 收尾（"提取得准不准"） |
| `ts` | server 当前 unix ms，客户端可作时钟校准基准 |

> 💡 `coreMemories` 是叙事性原文（"我们 3/13 凌晨初见"），`coreFacts` 是结构化事实（`relationship_with_mom=医生`）。两者互补：前者保留情境，后者密度高、塞 prompt 几乎不占 token。建议都注入到 system prompt 的"角色已经知道的事实"段。
