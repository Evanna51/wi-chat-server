...
服务端在 `/api/ws` 暴露一条 WebSocket 长连接，作为主动消息的实时下发通道，替代 `/api/pull-messages` 轮询。

### 11.1 协议

- URL: `ws://<host>:<port>/api/ws?apiKey=<APP_API_KEY>&userId=<userId>`，也可以放 header `x-api-key` + `x-user-id`
- 握手成功 server 立刻发 `{op: "hello"}`，紧跟着可能发一帧 `queued_batch` 把离线时段累积的 `local_outbox_messages` 一次性 flush
- `proactive` 帧 = 一条主动消息：`{op:"proactive", id, assistantId, sessionId, title, body, messageType, payload, createdAt}`
- 客户端收到后**必须**回一帧 `{op:"ack", id:"<message-id>", status:"received"}`，否则 server 上对应 outbox 行不会被标 `acked`
- 心跳：客户端每 25s 发 `{op:"ping", ts}`，server 回 `{op:"pong", ts}`
...
```kotlin
class ProactiveWsClient(
    private val baseUrl: String,        // ws://192.168.5.7:8787
    private val apiKey: String,
    private val userId: String,
    private val onMessage: (ProactiveFrame) -> Unit,
) {
    private val client = OkHttpClient.Builder()
        .pingInterval(25, TimeUnit.SECONDS)   // OkHttp 自动 RFC 6455 ping
        .build()
    ...
    fun connect() {
        val url = "$baseUrl/api/ws?apiKey=$apiKey&userId=$userId"
    ...
    private val listener = object : WebSocketListener() {
        override fun onOpen(webSocket: WebSocket, response: Response) {
            backoff = 1_000L
            // 让 server 重发一遍积压（可选，server 在 onConnect 已 flush 一次）
            webSocket.send("""{"op":"subscribe","userId":"$userId"}""")
        }
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            ...
            when (frame.op) {
                "hello", "pong" -> { /* no-op */ }
                "queued_batch" -> frame.messages?.forEach {
                    handleProactive(webSocket, it)
                }
                "proactive" -> handleProactive(webSocket, frame.toProactive())
                "server_shutdown" -> webSocket.close(1001, "server_shutdown")
            }
        }
    ...
    private fun handleProactive(ws: WebSocket, m: ProactiveFrame) {
        // 1. 写入本地 chat 表（按 messageType + assistantId + sessionId 入对应 conversation）
        ChatDao.insertProactiveMessage(m)
        // 2. 立刻 ack
        ws.send("""{"op":"ack","id":"${m.id}","status":"received"}""")
        // 3. 走通知 / 角色头像红点等 UI
        onMessage(m)
    }

    private fun scheduleReconnect() {
        Handler(Looper.getMainLooper()).postDelayed({ connect() }, backoff)
...
```
建立：
  app onResume → 触发 GET /api/health 探活
            ├─ 200 (家中 WiFi 可达) → ProactiveWsClient.connect()
            └─ 失败           → 不建连，依赖 daily sync + outbox

关闭：
  app onPause                          → close()
  ConnectivityManager.onLost           → close() + 等下次 onAvailable 重新探活
  ConnectivityManager 检测到 cellular  → close()（移动数据下不持长连）
  app 手动退出                          → close()
```

> 设计上**只在家中 WiFi 跑 WS**：移动数据下保持现有的 outbox + 下次 daily sync 拉取的兜底路径。这是为了省电 / 省流量 / 避免运营商 NAT 卡断。

### 11.4 重连指数退避

固定 1s → 2s → 4s → 8s → 16s → 30s 上限。OkHttp 自身没有重连机制，需要 listener 里 `onClosed` / `onFailure` 自己 schedule。

### 11.5 心跳

OkHttp `pingInterval(25, SECONDS)` 走 RFC 6455 frame 级 ping，server 那头我们也维护一个 25s pingInterval。**应用层**还可选发 `{op:"ping"}` 验证两端业务逻辑活着，但 OkHttp 的 ping 已经够用。

### 11.6 收到 proactive 帧的写库与 ack

```kotlin
ChatDao.insertProactiveMessage(
    PendingChatMessage(
        id = m.id,
        ...
        synced = 1,                  // server 推过来的，本地不需要再 push
        source = "proactive_ws",
    )
)
ws.send("""{"op":"ack","id":"${m.id}","status":"received"}""")
...
server 端的 `local_outbox_messages` 会在收到 ack 后置 `status='acked'`，下次重启 / 重连不会重复推。

### 11.7 自测清单

- [ ] 进入家中 WiFi → app 探活成功 → WS 建连，server stats `wsActiveSockets[<userId>] = 1`
- [ ] 连接期间收到一条 proactive → ack 后 server 端 `local_outbox_messages.status = 'acked'`
- [ ] 切到移动数据 → WS close → stats 清空
- [ ] 离线期间产生的 proactive plan 进了 outbox（status=pending）
- [ ] 回家 WiFi 再连上 → 立即收到 `queued_batch` 包含离线积压
- [ ] 进程被 SIGTERM → 客户端先收到 `server_shutdown`，再触发重连退避
...
- 设计文档：`docs/offline-sync-plan.md`、`docs/archive/realtime-and-autonomous-redesign.md`（角色自驱 + WS 演进背景）
- 服务端测试工具：`npm run sync:replay -- --mode test --assistant <id> --count N`（一键端到端，可作 reference 实现）
- WS 测试客户端：`npm run ws:test -- --user <userId>`
